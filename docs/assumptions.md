...
**Lei:** `PROMPT.md`  
**Perguntas ao dono:** `docs/11-open-questions.md`
...
| **A7** | WhatsApp: Meta Cloud API oficial via `WhatsAppProvider`. Sem WhatsApp Web / Puppeteer / bibliotecas não oficiais. |
| **A8** | IA: OpenAI Responses API + Structured Outputs + Zod. Modelo via ENV. Nunca ponto único de falha. |
| **A9** | Auth MVP: tabela `users` desde o dia 1 (`created_by`, audit). Papéis: `ADMIN` e `OPERATOR`. Primeiro usuário = administrador dono. Sessão por cookie httpOnly. Sem OAuth no MVP. |
...
| **A16** | Recorrência: uma `Task`, não clonar linhas por mês. `DeadlineRule` + `deadline_occurrences`. Completar um período registra a ocorrência e abre o próximo (volta a `PENDING`). |
...
| **A24** | Grupos de WhatsApp: não dependentes. Fallback: envio individual aos `NotificationTargets` e/ou mensagem pronta para copiar + in-app. |
...
| **A30** | `NotificationTargets` configuráveis para sócios (lista de pessoas/canais), não hardcoded. |
...
- AuthZ MVP: só `ADMIN` aprova prorrogação, confirma entrega e altera dependências.
